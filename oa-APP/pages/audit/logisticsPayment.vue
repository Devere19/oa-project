	...
	<view>
		...
		<view v-show="chooseForm==0" v-for="(item,index) in unauditedList" :key="item.id" class="formCard">
			<!-- <uni-transition custom-class="transition" :duration=500 :mode-class="item.modeClass" :show="item.show"> -->
				<uni-card style="box-shadow: 5rpx 5rpx 15rpx #00ffff" title="物流付款单" :extra="item.logisticsContractNo">
					...
					<view class="auditStatusGroup">
						<view v-for="(statusItem,statusIndex) in item.logisticsPaymentDirector" :key="statusItem.userId"
	...
	export default {
		...
		methods: {
			...
			searchClick(res) {
				uni.navigateTo({
					// 普通参数传输
					url: '/pages/search/searchLogisticsPayment?searchWord=' + res.value
				})
			},
			actionsClick(result, id, index) {
				if (result == "pass") {
					...
					uni.navigateTo({
						// 普通参数传输
						url: '/pages/detail/logisticsPaymentDetail?contractId=' + id
					})
				}
			},
		}
